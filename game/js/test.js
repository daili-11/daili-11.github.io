...
function Hero(w,h) {
	...
	hero.cacheCanvas = document.createElement("canvas");//创建canvas
	hero.cacheCtx = this.cacheCanvas.getContext("2d");//创建canvas链接
	hero.cacheCanvas.width = hero.w;   
    hero.cacheCanvas.height = hero.h;
	hero.draw = function (audio1,audio2) {
		...
		hero.cacheCtx.clearRect(0,0,hero.cacheCanvas.width,hero.cacheCanvas.height)
		hero.cacheCtx.drawImage(hero.imgs[hero.i],0,0)
		hero.cacheCtx.restore();
		ctx.drawImage(hero.cacheCanvas,hero.x,hero.y)
	}
}
//所有敌机
function Diji(w,h,type) {
	...
	obj.dI = 0;//判断延时用
	obj.arr = [];//存放敌人
	obj.cacheCanvas = document.createElement("canvas");//创建canvas
	obj.cacheCtx = this.cacheCanvas.getContext("2d");//创建canvas链接
	obj.cacheCanvas.width = obj.w;   
    obj.cacheCanvas.height = obj.h;
	obj.die = false;
	...
	obj.draw = function (hero,audio,audio1) {
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			audio1.play()//敌人飞行声音
			obj.cacheCtx.clearRect(0,0,obj.cacheCanvas.width,obj.cacheCanvas.height);
			obj.cacheCtx.drawImage(obj.imgs[obj.arr[i].i],0,0);
			obj.cacheCtx.restore();
			ctx.drawImage(obj.cacheCanvas,obj.arr[i].x,obj.arr[i].y)
			//如果敌机没挂，判断敌机与玩家飞机碰撞
			if (!obj.arr[i].die) {
...
function Bullet(w,h,ws) {
	...
	Bu.cacheCanvas = document.createElement("canvas");//创建canvas
	Bu.cacheCtx = this.cacheCanvas.getContext("2d");//创建canvas链接
	Bu.cacheCanvas.width = Bu.w;   
    Bu.cacheCanvas.height = Bu.h;
	Bu.draw = function (hero,arr,audio) {
		...
		for (var i = 0; i < Bu.obs.length; i++) {
			...
			audio.play()//子弹打出声音
			if (Bu.change && Bu.changeDI<5000) {
				Bu.cacheCtx.clearRect(0,0,Bu.cacheCanvas.width,Bu.cacheCanvas.height);
				Bu.cacheCtx.drawImage(Bu.img,0,0);
				Bu.cacheCtx.restore();

				Bu.changeDI++;ctx.drawImage(Bu.imgs,hero.x+hero.w/2-Bu.ws/2,Bu.obs[i].y)
			}
			else {
				...
				Bu.cacheCtx.drawImage(Bu.img,0,0);
				Bu.cacheCtx.restore();
				ctx.drawImage(Bu.cacheCanvas,Bu.obs[i].x,Bu.obs[i].y)}
		}
